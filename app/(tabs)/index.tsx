...
// ─── Ghost Message Component ──────────────────────────────────────────────
const GhostMessage = React.memo(({ content }: { content: string }) => {
  ...
  // Check for raw object dumps
  if (content === "[object Object]" || content.trim().startsWith("{")) {
    return (
      <Text
        style={{
          color: C.warn,
      ...
      >
        ⚠️ Received raw object. Check server logs.
      </Text>
    );
  }
  return (
        ...
        <View style={s.slashSheet}>
          {SLASH_COMMANDS.map((sc, i) => (
            <TouchableOpacity
              ...
              onPress={() => {
                if (
                  ["/help", "/clear", "/reset", "/status"].includes(sc.command)
                ) {
                  setInput("");
                  setShowSlash(false);
                  doSend(sc.command);

    ...
    anims.forEach((a, i) =>
      Animated.loop(
        Animated.sequence([
          ...
          Animated.timing(a.current, { toValue: 1, duration: 320, useNativeDriver: true }),
          Animated.timing(a.current, { toValue: 0, duration: 320, useNativeDriver: true }),
        ]),
      ).start(),
    );
  }, []);
...
function RecordingDot() {
  ...
  useEffect(() => {
    Animated.loop(Animated.sequence([
      Animated.timing(pulse, { toValue: 1.4, duration: 550, useNativeDriver: true }),
      Animated.timing(pulse, { toValue: 1,   duration: 550, useNativeDriver: true }),
    ])).start();
  }, []);
  return <Animated.View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: C.danger, transform: [{ scale: pulse }] }} />;
}
...
const mkStyles: Record<string, any> = {
  ...
  heading1: { color: "#FFFFFF", fontWeight: "700", fontSize: 22, marginTop: 18, marginBottom: 8 },
  heading2: { color: "#FFFFFF", fontWeight: "700", fontSize: 19, marginTop: 16, marginBottom: 6 },
  heading3: { color: C.textPrimary, fontWeight: "600", fontSize: 17, marginTop: 12, marginBottom: 4 },
  // Emphasis
  ...
  bullet_list_icon:  { color: C.accent, fontSize: 16, marginRight: 10, lineHeight: 26 },
  ordered_list_icon: { color: C.accent, fontSize: 15, marginRight: 10, fontWeight: "700", lineHeight: 26 },
  // Link
  link: { color: C.accent, textDecorationLine: "underline" },
  // HR
  hr: { backgroundColor: C.border, height: 1, marginVertical: 14 },
  // Tables
  table: { borderWidth: 1, borderColor: C.border, borderRadius: 8, marginVertical: 10, overflow: "hidden" },
  thead: { backgroundColor: "#0D1117" },
  tr:    { borderBottomWidth: 1, borderBottomColor: C.border },
  th:    { color: "#FFF", fontWeight: "700", fontSize: 14, padding: 12, borderRightWidth: 1, borderRightColor: C.border },
  td:    { color: C.textPrimary, fontSize: 14, lineHeight: 20, padding: 10, borderRightWidth: 1, borderRightColor: C.border },
};

// ─── Status icon (user messages only) ────────────────────────────────────
function StatusIcon({ status }: { status?: string }) {
  if (status === "sending")   return <Text style={s.statusIcon}>⏱</Text>;
  if (status === "completed") return <Text style={[s.statusIcon, { color: C.accent }]}>✓</Text>;
  if (status === "failed")    return <Text style={[s.statusIcon, { color: C.danger }]}>✗</Text>;
  return null;
}

// ─── Connection badge ─────────────────────────────────────────────────────
function ConnectionBadge({ state }: { state: ConnectionState }) {
  const color = state === "online" ? C.accent : state === "syncing" ? C.warn : C.danger;
  const label = state === "online" ? "ONLINE" : state === "syncing" ? "SYNCING" : "OFFLINE";
  return (
    <View style={{ flexDirection: "row", alignItems: "center", gap: 5 }}>
      <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: color }} />
      <Text style={{ color, fontSize: 9, fontWeight: "700", letterSpacing: 1.5, fontFamily: FONT_MONO }}>{label}</Text>
    </View>
  );
}
          ...
          {msg.media_url && (
            <View style={{ marginBottom: 8 }}>
              {msg.media_type?.startsWith("image/") ? (
                <Image source={{ uri: msg.media_url }} style={s.attachedImage} resizeMode="cover" />
              ) : (
    ...
    <View style={s.ghostRow}>
      ...
      <View style={s.ghostContent}>
        {isPlaceholder ? (
          ...
          <>
            <Markdown style={mkStyles} rules={markdownRules}>{content}</Markdown>
            <Text style={s.ts}>{timeStr}</Text>
          </>
        )}
      </View>
...
// ─── Search bar ───────────────────────────────────────────────────────────
function SearchBar({ query, onChangeQuery, onClose, results }: {
  query: string; onChangeQuery: (q: string) => void; onClose: () => void; results: number;
}) {
  return (
    <View style={s.searchWrap}>
      <Text style={{ color: C.textSecondary, fontSize: 15 }}>⌕</Text>
      <TextInput
        style={s.searchInput} value={query} onChangeText={onChangeQuery}
        placeholder="Search messages…" placeholderTextColor={C.textTertiary}
        autoFocus
      />
      {query.length > 0 && <Text style={{ color: C.textSecondary, fontSize: 11 }}>{results} found</Text>}
      <TouchableOpacity onPress={onClose}>
        <Text style={{ color: C.accent, fontSize: 12, fontWeight: "700", letterSpacing: 0.5 }}>DONE</Text>
      </TouchableOpacity>
    </View>
  );
}
...
function ToolStatusBadge({ label }: { label: string }) {
  ...
  return (
    <Animated.View style={[toolBadgeStyles.wrap, { opacity: pulse }]}>
      ...
      <Text style={toolBadgeStyles.text} numberOfLines={1}>{label}</Text>
    </Animated.View>
  );
}
...
export default function ChatScreen() {
  ...
  const {
    ...
    setMessages, setLastSentMessage, lastSentMessage,
    removeMessage, updateMessageStatus, enqueueMessage, dequeueMessages,
    _lastCommitTime, _lastCommitContent,
  } = useGhostStore();

  const [input, setInput]               = useState("");
  const [pendingMedia, setPendingMedia] = useState<{ uri: string; b64: string; mimeType: string; name?: string } | null>(null);
  ...
  useEffect(() => {
    if (!config) return;
    fetchHistory(config, 60, 0).then(d => {
      setMessages([...d.messages].reverse().map(m => ({ ...m, status: "completed" as const })));
    ...
    return onWSMessage(msg => {
      ...
      if (st._lastCommitTime && Date.now() - st._lastCommitTime < 10_000) {
        ...
        if (ln === incoming || ln.includes(incoming) || incoming.includes(ln)) return;
      }
      const last = st.messages[st.messages.length - 1];
      if (last?.role === "assistant" && normalize(last.content) === incoming) return;
      appendMessage({ id: uid(), role: "assistant", content: incoming, timestamp: Date.now() / 1000, status: "completed" });
    });
  }, [config]);
  ...
  const loadOlder = useCallback(async () => {
    ...
    try {
      ...
      setMessages([...[...d.messages].reverse().map(m => ({ ...m, status: "completed" as const })), ...messages]);
      setTotalMessages(d.total);
    } catch {}
  ...
  const doSend = useCallback(async (text: string, mediaB64?: string, mediaType?: string, mediaUri?: string) => {
    ...
    const msgId = uid();
    appendMessage({ id: msgId, role: "user", content: text || "📎 Attachment", timestamp: Date.now() / 1000, media_url: mediaUri, media_type: mediaType, status: "sending" });
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    appendMessage(createStreamingPlaceholder());
    setStreaming(true);
    setActiveError(null);
    setLastSentMessage({ content: text, mediaB64, mediaType });
    streamTimeout.current = setTimeout(() => { if (useGhostStore.getState().isStreaming) commitStream(); }, 30_000);
    ...
    await sendMessage(config, {
      ...
      onError: err => {
        ...
        if (last?.role === "assistant" && !last.content.trim()) removeMessage(last.id);
        setActiveError({ error: err, partialContent: partial?.trim() || undefined });
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
      },
    });
  ...
  const handleSend = useCallback(async () => {
    ...
    const text = input.trim(), media = pendingMedia;
    setInput(""); setPendingMedia(null);
    if (connectionState === "offline") {
      enqueueMessage({ content: text, mediaB64: media?.b64, mediaType: media?.mimeType });
      appendMessage({ id: uid(), role: "user", content: text || "📎 Attachment", timestamp: Date.now() / 1000, media_url: media?.uri, status: "sending" });
      return;
    }
  ...
  const pickImage = async () => {
    const r = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, base64: true, quality: 0.8 });
    if (!r.canceled && r.assets[0].base64) setPendingMedia({ uri: r.assets[0].uri, b64: r.assets[0].base64, mimeType: r.assets[0].mimeType ?? "image/jpeg", name: r.assets[0].fileName ?? "image.jpg" });
  };
  const pickDoc = async () => {
    if (!config) return;
    ...
    if (!r.canceled && r.assets[0]) {
      const a = r.assets[0];
      const { b64, mime_type } = await uploadFile(config, a.uri, a.mimeType ?? "application/octet-stream", a.name);
  ...
  const stopRec = async () => {
    ...
    if (uri) {
      ...
      setInput(p => t ? (p ? `${p} ${t}` : t) : `${p} [Voice — unavailable]`);
    }
  ...
  const handleClear = useCallback(() => {
    if (!config) return;
    if (isStreaming) { if (streamTimeout.current) clearTimeout(streamTimeout.current); commitStream(); }
    Alert.alert("Clear chat?", "Archives mobile history. Ghost's memory is unaffected.", [
      { text: "Cancel", style: "cancel" },
      { text: "Clear", style: "destructive", onPress: async () => {
        try { await clearChat(config); setMessages([]); setActiveError(null); setTotalMessages(0); }
        catch { setActiveError({ error: { kind: "network", message: "Failed to clear chat", retryable: false } }); }
      }},
    ]);
  ...
  const toggleAttach = () => {
    ...
    Animated.spring(attachAnim, { toValue: to, useNativeDriver: true, tension: 120, friction: 10 }).start();
  };
  const closeAttach = () => {
    setAttachOpen(false);
    Animated.spring(attachAnim, { toValue: 0, useNativeDriver: true, tension: 120, friction: 10 }).start();
  };
  ...
  if (!config) {
    return (
      <View style={[s.container, s.centered, { paddingTop: insets.top }]}>
        ...
        <Text style={s.noConfigSub}>Go to ⚙️ Settings to connect to your Pi</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      ...
      <View style={[s.header, { paddingTop: insets.top + 8 }]}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          <TouchableOpacity style={s.headerBtn} onPress={() => setSearchVisible(v => !v)}>
            <Text style={{ color: C.textSecondary, fontSize: 17 }}>⌕</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[s.clearBtn, isStreaming && { opacity: 0.35 }]}
            onPress={handleClear} disabled={isStreaming}
          >
      ...
      <FlatList
        ...
        ListHeaderComponent={
          loadingOlder ? (
            ...
            <TouchableOpacity style={{ padding: 14, alignItems: "center" }} onPress={loadOlder}>
              <Text style={{ color: C.accent, fontSize: 12, fontWeight: "600" }}>↑ Load older</Text>
            </TouchableOpacity>
          ) : null
        }
        ListFooterComponent={
      ...
      {pendingMedia && (
        <View style={s.mediaPreview}>
          {pendingMedia.mimeType?.startsWith("image/")
            ...
            : <View style={[s.mediaThumb, { backgroundColor: C.surface2, alignItems: "center", justifyContent: "center" }]}><Text style={{ fontSize: 18 }}>📄</Text></View>
          }
          <Text style={s.mediaName} numberOfLines={1}>{pendingMedia.name ?? "Attachment"}</Text>
          <TouchableOpacity onPress={() => setPendingMedia(null)} style={s.mediaRemove}>
            ...
            {SLASH_COMMANDS.filter((sc) => {
              ...
              .map((sc, i, arr) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (
                      ["/help", "/clear", "/reset", "/status", "/skills", "/tools"].includes(sc.command)
                    ) {
                      setInput(""); setShowSlash(false); doSend(sc.command);
                    } else if (sc.command === "/install") {
                      setInput("/install "); setShowSlash(false);
                      setTimeout(() => inputRef.current?.focus(), 50);
                    } else {
                      setInput(sc.command + " "); setShowSlash(false);
                      setTimeout(() => inputRef.current?.focus(), 50);
                    }
                  }}
                >
      ...
      {/* ── Attach tray ── */}
      {attachOpen && (
        <Animated.View style={[s.attachTray, {
          opacity: attachAnim,
          transform: [{ translateY: attachAnim.interpolate({ inputRange: [0, 1], outputRange: [10, 0] }) }],
        }]}>
          {[
            { icon: "🖼", label: "Photo", action: () => { pickImage(); closeAttach(); } },
            { icon: "📄", label: "File",  action: () => { pickDoc();   closeAttach(); } },
          ].map(item => (
            <TouchableOpacity key={item.label} style={s.attachItem} onPress={item.action}>
              <View style={s.attachIconWrap}><Text style={{ fontSize: 22 }}>{item.icon}</Text></View>
      ...
      {/* ── Input bar ── */}
      <View style={[s.inputBar, { paddingBottom: Platform.OS === "ios" ? Math.max(insets.bottom, 8) + 2 : 8 }]}>
        {/* + button */}
        <TouchableOpacity
        ...
        >
          <Animated.Text style={[s.circleBtnIcon, {
        ...
        {isRecording || isTranscribing ? (
          <View style={s.recPill}>
            {isTranscribing ? (
              <><ActivityIndicator size="small" color={C.warn} /><Text style={[s.recText, { color: C.warn }]}>Transcribing…</Text></>
            ) : (
              <><RecordingDot /><Text style={[s.recText, { color: C.danger }]}>{fmtDur(recordDuration)}</Text><Text style={s.recHint}>tap ⏹ to stop</Text></>
            )}
          </View>
        ) : (
        ...
        {isRecording ? (
          ...
          <TouchableOpacity
            style={[s.sendBtn, (isStreaming || isTranscribing || (!input.trim() && !pendingMedia)) && s.sendBtnOff]}
            onPress={handleSend}
...
const s = StyleSheet.create({
  ...
  headerTitle: {
    color: C.accent, fontSize: 16, fontWeight: "800",
    letterSpacing: 7, fontFamily: FONT_MONO,
  },
  headerBtn: {
  ...
  clearBtn: {
    ...
    borderWidth: 1, borderColor: "#00FF8835",
  },
  clearBtnTxt: {
    color: C.accent, fontSize: 10, fontWeight: "700",
    letterSpacing: 1.2, fontFamily: FONT_MONO,
  },
  ...
  searchInput: {
    ...
    fontFamily: FONT_SANS, paddingVertical: 0,
  },
  ...
  mediaRemove: {
    ...
    backgroundColor: "#FF445518", alignItems: "center", justifyContent: "center",
  },

  // Offline
  offlineBanner: {
    backgroundColor: "#13110A", borderTopWidth: 1, borderTopColor: "#2A2510",
    paddingHorizontal: 16, paddingVertical: 8, alignItems: "center",
  },
  offlineText: { color: C.warn, fontSize: 12, fontWeight: "600", fontFamily: FONT_SANS },
  ...
  slashCmd:  { color: C.accent, fontSize: 13, fontWeight: "700", fontFamily: FONT_MONO, minWidth: 72 },
  slashDesc: { color: C.textSecondary, fontSize: 13, flex: 1, fontFamily: FONT_SANS },
  ...
  attachIconWrap: {
    ...
    backgroundColor: "#0D1A24", borderWidth: 1, borderColor: C.border,
    alignItems: "center", justifyContent: "center",
  },
  attachLabel: { color: C.textSecondary, fontSize: 11, fontWeight: "600", fontFamily: FONT_SANS },
  ...
  circleBtnIcon: { color: C.textSecondary, fontSize: 22, fontWeight: "200", lineHeight: 26, includeFontPadding: false },
  ...
  recPill: {
    ...
    borderRadius: 22, borderWidth: 1, borderColor: "#2A1010",
    minHeight: 44, paddingHorizontal: 14, paddingVertical: 10,
  },
  recText: { fontSize: 14, fontWeight: "700", letterSpacing: 0.5, fontFamily: FONT_MONO },
  recHint: { color: C.textSecondary, fontSize: 11, marginLeft: "auto" as any, fontFamily: FONT_SANS },
  ...
  stopSquare: { width: 11, height: 11, borderRadius: 2, backgroundColor: C.danger },
  sendBtn: {
    width: 38, height: 38, borderRadius: 19,
    backgroundColor: C.accent,
    alignItems: "center", justifyContent: "center",
